<?php
	...
	class userblog extends base {
					...
					if(is_array($_FILES)) {
						foreach($_FILES as $k => $d) {
							...
							if($d['error'] != 4) {
								...
								$ext = pathinfo($d['name']);
								$exrarr = explode(',', $this -> luomor['theme_uploadtype']);
								if(!in_array($ext['extension'], $exrarr)) {
									exit('只能上传：' . $this -> luomor['theme_uploadtype']);
								}
		...
		private function getMyFollow() {
			//显示我关注的24个
			$this -> follow = spClass('follow') -> spLinker() -> findAll(array('uid' => $this -> user_data['uid']), 'time desc', '', '24');
		}
		...
		private function getBlogFava() {
			...
			$count = spClass('likes') -> findCount(array('bid' => $bid));
			$data['rs'] = $rs;
			$data['count'] = $count;
			return $data;
		}
		...
		public function display($tplname, $rs) {
			...
			//如果没定义domain 就是home
			$this -> domain = ($this -> user_data['domain'] == '') ? 'home' : $this -> user_data['domain'];
