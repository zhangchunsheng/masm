<?php
	...
	function spAccess($method, $name, $value = NULL, $life_time = -1) {
		...
		if($launch = spLaunch("function_access",
							array('method' => $method, 'name' => $name, 'value' => $value, 'life_time' => $life_time), TRUE))
			return $launch;
		...
		if($method == 'w') {
			...
			$life_time = ($life_time == -1) ? '300000000' : $life_time;
			// 准备存入缓存文件的数据，缓存文件使用PHP的die();函数以便保证内容安全，
			$value = '<?php die();?>' . ($life_time + time()) . serialize($value); // 数据被序列化后保存
	...
	function spClass($class_name, $args = null, $sdir = null, $force_inst = FALSE) {
		...
		if($sdir != null && !import($sdir) && !import($sdir . '/' . $class_name . '.php'))
			return FALSE;
