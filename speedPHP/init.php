<?php
	...
	if($GLOBALS['G_SP']['url']["url_path_info"] == TRUE && !empty($_SERVER['PATH_INFO'])) {
	...
	//加载用户级设置变量
	if(!spAccess('r', 'siteconfig')) { //读取设置
		$siteconfig = spClass('site_setting') -> getConfig();
		spAccess('w', 'siteconfig', $siteconfig, 36000);
	} else {
		$siteconfig = spAccess('r', 'siteconfig');
	}
	...
	// 自动执行用户代码
	if($GLOBALS['G_SP']['auto_sp_run'] == TRUE)
		spRun();
