	...
	if (data.err != undefined && data.msg != undefined) {
		...
		if (!data.err) {
			...
			if (urls.length == 2) {
				...
				var url = urls;
			} //如果返回的是带缩略图的就让0进去，否则直接进去

			$('#uploadArea').append(' <div id="attach_' + data.msg.fid + '"><img src="' + urlpath + '/' + url + '" width="100" height="100" /><textarea name="localimg[' + data.msg.fid + ']" onclick="upimgclk(this)">图片说明可选</textarea>' + '<a href="javascript:void(0)" onclick="delAttachIMAGE(' + data.msg.fid + ')" class="delete">remove</a></div>');
