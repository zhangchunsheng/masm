...
(function ($, undefined) {
	...
	$.xheditor = function (textarea, options) {
		...
		var headHTML = '',
			...
			emotPath = emotPath ? emotPath : editorRoot + 'e/',
			selEmotGroup = '';
		...
		this.init = function () {
			...
			if(editorBackground)
				iframeHTML += '<style>body{background:' + editorBackground + ';}</style>';
			...
			if(settings.submitID)
				$('#' + settings.submitID).click(saveResult); //自定义绑定submit按钮
			...
			//HTML5编辑区域直接拖放上传
			if(settings.html5Upload)
				jBody.bind('dragenter dragover', function (ev) {
					var types;
					if((types = ev.originalEvent.dataTransfer.types) && $.inArray('Files', types) !== -1)
						return false;
					...
					if(dataTransfer && (fileList = dataTransfer.files) && fileList.length > 0) {
						...
						for(i in arrCmd) {
							cmd = arrCmd[i];
							if(settings['up' + cmd + 'Url'] && settings['up' + cmd + 'Url'].match(/^[^!].*/i))
								arrExt.push(cmd + ':,' + settings['up' + cmd + 'Ext']); //允许上传
						}
						if(arrExt.length === 0)
							return false; //禁止上传
						else
						...
						cmd = getCmd(fileList);
						if(cmd === 1)
							alert('上传文件的扩展名必需为：' + strExt.replace(/\w+:,/g, ''));
						else if(cmd === 2)
							alert('每次只能拖放上传同一类型文件');
						else if(cmd) {
							_this.startUpload(fileList, settings['up' + cmd + 'Url'], '*', function (arrMsg) {
								var arrUrl = [],
									msg, onUpload = settings.onUpload;
								if(onUpload)
									onUpload(arrMsg); //用户上传回调
								for(i in arrMsg) {
									msg = arrMsg[i];
		...
		this.focus = function () {
			...
			if(isIE) {
				...
				if(rng.parentElement && rng.parentElement().ownerDocument !== _doc)
					_this.setCursorFirst(); //修正IE初始焦点问题
			}
			return false;
		}
				...
				sHtml = sHtml.replace(/(<(\w+))((?:\s+[\w-:]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*(\/?>)/g, function (all, left, tag, attr, right) {
					...
					attr = attr.replace(/\s+([\w-:]+)\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/g, function (all, n, v) {
						...
						if(n === 'class') { //清理class属性
							...
							if(v === '')
								return '';
						} else if(n.match(/^((_xhe_|_moz_|_webkit_)|jquery\d+)/i))
							return ''; //清理临时属性					
						else if(saveValue && n.match(/^(src|href)$/i))
							return ' ' + n + '="' + saveValue + '"'; //恢复属性值src,href
		...
		this.cleanWord = function (sHtml) {
			...
			if(cleanPaste > 0 && cleanPaste < 3 && sHtml.match(/mso(-|normal)|WordDocument|<table\s+[^>]*?x:str/i)) {
				...
				sHtml = sHtml.replace(/(<(\/?)([\w-:]+))((?:\s+[\w-:]+(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))?)*)\s*(\/?>)/g, function (all, left, end, tag, attr, right) {
					tag = tag.toLowerCase();
					if((tag.match(/^(link|img)$/) && attr.match(/file:\/\//i)) || tag.match(/:/) || (tag === 'span' && cleanPaste === 2))
						return '';
					if(!end) {
						attr = attr.replace(/\s([\w-:]+)(?:\s*=\s*("[^"]*"|'[^']*'|[^>\s]+))?/ig, function (all, n, v) {
							...
							if(n.match(/:/))
								return '';
							else if(n.match(/^(class|lang|language|span)$/))
								return '';
							else if(tag === 'td' && (n === 'height' || (n === 'width' && !attr.match(/\scolspan="\d+"/i))))
								return '';
							else if(n === 'style') {
								if(cleanPaste === 2)
									return '';
		...
		this.formatXHTML = function (sHtml, bFormat) {
			...
			while(match = regTag.exec(sHtml)) {
				...
				if(tagCDATA)
					...
					if(tagName = match[3]) { //开始标签
						...
						onComment(match[0]); //注释标签
				}

			}
			if(sHtml.length > nextIndex)
				onText(sHtml.substring(nextIndex, sHtml.length)); //结尾文本
			onEndTag(); //封闭未结束的标签
			...
			function onStartTag(tagName, rest, unary) {
				if(blockTags[tagName])
					while(stack.last() && inlineTags[stack.last()])
						onEndTag(stack.last()); //块标签
				if(closeSelfTags[tagName] && stack.last() === tagName)
					onEndTag(tagName); //自封闭标签
			...
			function font2style(all, tag, attrs, content) {
				...
				if(s)
					styles += 'font-size:' + arrFontsize[(s[1] > 7 ? 7 : (s[1] < 1 ? 1 : s[1])) - 1].s + ';';
		...
		this.toggleSource = function (state) {
			...
			if(!bSource) { //转为源代码模式
				...
				if(!isOpera)
					cursorPos = sHtml.substring(0, cursorPos).replace(/\r/g, '').length; //修正非opera光标定位点
			...
			if(bSource) {//光标定位源码
				...
				_this.setCursorFirst(true); //定位最前面
		...
		this.showLink = function () {
			...
			if(settings.upLinkUrl)
				_this.uploadInit(jUrl, settings.upLinkUrl, settings.upLinkExt);
			jSave.click(function () {
				_this.loadBookmark();
				var url = jUrl.val();
				...
				if(url !== '' && url !== 'http://') {
					...
					if(aUrl.length > 1) { //批量插入
						...
						} else if(sText && !isSafari)
							jParent.text(sText); //safari改写文本会导致光标丢失
		...
		this.showAnchor = function () {
			...
			jSave.click(function () {
				...
				if(name) {
					if(jParent.length === 0)
						_this.pasteHTML('<a name="' + name + '"></a>');
					else
						jParent.attr('name', name);
				} else if(jParent.length === 1)
					jParent.remove();
				_this.hidePanel();
				return false;
			});
		...
		this.showImg = function () {
			...
			if(settings.upImgUrl)
				_this.uploadInit(jUrl, settings.upImgUrl, settings.upImgExt);
			jSave.click(function () {
				_this.loadBookmark();
				var url = jUrl.val();
				if(url !== '' && url !== 'http://') {
		...
		this.showEmbed = function (sType, sHtml, sMime, sClsID, sBaseAttrs, sUploadUrl, sUploadExt) {
			...
			if(sUploadUrl)
				_this.uploadInit(jUrl, sUploadUrl, sUploadExt);
			...
			if(toUrl.substr(0, 1) === '!') {//自定义上传管理页
				...
				setTimeout(function () { //拖放上传
					jText.closest('.xheDialog').bind('dragenter dragover', returnFalse).bind('drop', function (ev) {
						var dataTransfer = ev.originalEvent.dataTransfer,
							fileList;
						if(bHtml5Upload && dataTransfer && (fileList = dataTransfer.files) && fileList.length > 0)
							_this.startUpload(fileList, toUrl, upext, setUploadMsg);
						return false;
					});
				}, 10);
			}
		...
		this.startUpload = function (fromFiles, toUrl, limitExt, onUploadComplete) {
			...
			if(!bHtml5Upload || (fromFiles.nodeType && !((fileList = fromFiles.files) && fileList[0]))) {
				...
				for(i = 0; i < len; i++)
					if(!checkFileExt(fileList[i].fileName, limitExt))
						return;
			...
			function onUploadCallback(sText, bFinish) {
				...
				if(!bOK || bFinish)
					_this.removeModal();
				if(bFinish && bOK)
					onUploadComplete(arrMsg); //全部上传完成
				return bOK;
			}
		}
		...
		this.html5Upload = function (inputname, fromFiles, toUrl, callback, onProgress) {
			...
			this.uploadNext = function (sText) {
				...
				if((!sText || (sText && callback(sText, i === count) === true)) && i < count)
					postFile(fromFiles[i++], toUrl, _this.uploadNext, function (loaded) {
		...
		this.showIframeModal = function (title, url, callback, w, h, onRemove) {
			var jContent = $('<iframe frameborder="0" src="' + url.replace(/{editorRoot}/ig, editorRoot) + (/\?/.test(url) ? '&' : '?') + 'editorhost=' + location.hostname + '" style="width:100%;height:100%;display:none;" /><div class="xheModalIfmWait"></div>'),
				jIframe = $(jContent[0]),
				jWait = $(jContent[1]);
			...
			function initModalWin() {
				try {
					...
					if((result = modalWin.name) === undefined)
						bCross = true;
				} catch (ex) {
					bCross = true;
				}
		...
		this.showModal = function (title, content, w, h, onRemove) {
			...
			if(layerShadow > 0)
				jModalShadow = $('<div class="xheModalShadow" style="width:' + jModal.outerWidth() + 'px;height:' + jModal.outerHeight() + 'px;margin-left:-' + (Math.ceil(w / 2) - layerShadow - 2) + 'px;' + (isIE && browerVer < 7.0 ? '' : 'margin-top:-' + (Math.ceil(h / 2) - layerShadow - 2) + 'px') + '"></div>').appendTo('body');
		...
		this.showPanel = function (content) {
			...
			if((x + _jPanel.outerWidth()) > ((window.pageXOffset || _docElem.scrollLeft || body.scrollLeft) + (_docElem.clientWidth || body.clientWidth)))
				x -= (_jPanel.outerWidth() - _jPanelButton.outerWidth()); //向左显示面板
		...
		this.exec = function (cmd) {
			...
			if(ev === null) {//非鼠标点击
				...
				if(btn.length === 1)
					ev.target = btn; //设置当前事件焦点
			}
			if(tool.e) {
				tool.e.call(_this) //插件事件
			} else {//内置工具
				cmd = cmd.toLowerCase();
				switch (cmd) {
					case 'cut':
	...
	$(function () {
		...
		$.fn.val = function (value) {
			var _this = this,
				editor;
			if(value === undefined)
				if(_this[0] && (_this[0].xheditor = editor))
					return editor.getSource();
			else
				return _this.oldVal(); //读
